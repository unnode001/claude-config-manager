...
use claude_config_manager_core::{merge_configs, ClaudeConfig, ConfigManager};
use std::fs;
...
fn bench_config_parsing() {
    ...
    assert!(
        ...
        "Config parsing took {duration:?}, expected < 10ms"
    );
}
...
fn bench_config_writing() {
    ...
    assert!(
        ...
        "Config writing took {duration:?}, expected < 50ms"
    );
}
...
fn bench_config_merging() {
    ...
    assert!(
        ...
        "Config merging took {duration:?}, expected < 5ms"
    );
}
...
fn bench_large_config_parsing() {
    ...
    assert!(
        duration.as_millis() < 50,
        "Large config parsing took {duration:?}, expected < 50ms"
    );
}
...
fn bench_repeated_parse_write_cycle() {
    ...
    println!("10 parse-write cycles took: {duration:?} (avg: {avg_duration:?} per cycle)");

    // Each cycle should be fast (write + read)
    assert!(
        avg_duration.as_millis() < 20,
        "Average cycle took {avg_duration:?}, expected < 20ms"
    );
}
